def generate_response(user_message, latest_detection, latest_summary):
    ...
    if user_message in ['help', '/help']:
        return ("I can provide information about the current safety situation. You can ask me about:\n"
    ...
    elif any(word in user_message for word in ['recommendation', 'action', 'what should we do']):
        ...
        hardhat_compliance = latest_detection.get('Hardhat', 0) / (latest_detection.get('Hardhat', 0) + latest_detection.get('NO-Hardhat', 0)) * 100 if (latest_detection.get('Hardhat', 0) + latest_detection.get('NO-Hardhat', 0)) > 0 else 0
        vest_compliance = latest_detection.get('Safety Vest', 0) / (latest_detection.get('Safety Vest', 0) + latest_detection.get('NO-Safety Vest', 0)) * 100 if (latest_detection.get('Safety Vest', 0) + latest_detection.get('NO-Safety Vest', 0)) > 0 else 0
        overall_compliance = (hardhat_compliance + vest_compliance) / 2
        return (f"Current PPE compliance rates:\n"
        ...
        return ("I'm not sure how to answer that. You can ask me about the number of people detected, "
